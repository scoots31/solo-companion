...
def _render_feed_events(events, proj_meta=None):
    ...
    for date in sorted(days.keys(), reverse=True):
        ...
        html += (
            "<div class='feed-day' style='margin-bottom:32px;'>"
...
def activity_feed():
    ...
    feed_js = (
        ...
        # state
        "var _ap='all',_at='all';"
        # apply both filters with AND logic; hide empty day groups
        "function _applyFilters(){"
        "  document.querySelectorAll('.event[data-project]').forEach(function(ev){"
        "    var mp=_ap==='all'||ev.dataset.project===_ap;"
        "    var mt=_at==='all'||ev.dataset.type===_at;"
        "    ev.style.display=(mp&&mt)?'flex':'none';"
        "  });"
        "  document.querySelectorAll('.feed-day').forEach(function(grp){"
        "    var vis=Array.from(grp.querySelectorAll('.event[data-project]'))"
        "      .some(function(e){return e.style.display!=='none';});"
        "    grp.style.display=vis?'block':'none';"
        "  });"
        "}"
        # dropdown toggle
        ...
        "    _ap=opt.dataset.project;"
        "    document.getElementById('proj-dd-label').textContent=_ap==='all'?'All projects':_ap;"
        "    document.getElementById('proj-dd-dot').style.background=opt.dataset.color||'rgba(255,255,255,0.3)';"
        "    document.getElementById('proj-dropdown').style.display='none';"
        "    _applyFilters();"
        "  });"
        "});"
        # type chip click — single-select; click active chip again to reset to All
        "document.querySelectorAll('.type-chip').forEach(function(chip){"
        "  chip.addEventListener('click',function(){"
        "    var val=chip.dataset.type;"
        "    _at=(val==='all'||_at===val)?'all':val;"
        "    document.querySelectorAll('.type-chip').forEach(function(c){"
        "      var active=(c.dataset.type==='all'&&_at==='all')||(c.dataset.type===_at&&_at!=='all');"
        "      c.style.background=active?'rgba(37,99,235,0.15)':'rgba(255,255,255,0.05)';"
        "      c.style.borderColor=active?'rgba(37,99,235,0.3)':'rgba(255,255,255,0.08)';"
        "      c.style.color=active?'#93C5FD':'rgba(255,255,255,0.4)';"
        "    });"
        "    _applyFilters();"
        "  });"
        "});"
